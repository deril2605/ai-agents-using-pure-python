# ------------------------------------------------------------------------------
# Routing Pattern for Calendar Request Processing
# ------------------------------------------------------------------------------
...
# │  - Uses LLM to classify the request as:      │
...
# │  - If confidence_score < 0.7 → STOP 🚫        │
# │  - Else → Continue to Step 2                  │
# └───────────────────────────────────────────────┘
#                                                 ↓
# ┌───────────────────────────────────────────────┐
# │  Step 2: Process Request Based on Type        │
# │  - "new_event" → Extract event details        │
# │  - "modify_event" → Extract modification data │
# │  - "other" → Return "unsupported request"     │
# └───────────────────────────────────────────────┘
#                                                 ↓
# ┌───────────────────────────────────────────────┐
# │  Step 3: Generate Response                    │
# │  - If "new_event" → Generate confirmation     │
# │  - If "modify_event" → Generate update msg    │
# │  - Returns structured CalendarResponse        │
# └───────────────────────────────────────────────┘
#                                                 ↓
# ┌───────────────────────────────────────────────┐
# │  Final Output:                                │
# │  - If valid request → Print confirmation msg  │
...
def process_calendar_request(user_input: str) -> Optional[CalendarResponse]:
    ...
    if route_result.confidence_score < 0.7:
        ...
        return None
    print(route_result.request_type)
